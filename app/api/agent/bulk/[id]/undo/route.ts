...
import { requestBulkUndo } from "@/lib/agent/bulk/service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: "Invalid undo request" }, { status: 400 });
    const { id } = await context.params;
    const command = await requestBulkUndo(prismaBulkCommandRepository, auth.session.userId, id, parsed.data.digest);
