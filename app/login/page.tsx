...
export default function LoginPage() {
  ...
  const [localDevelopmentAuth, setLocalDevelopmentAuth] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    void fetch("/api/auth/local-development")
      .then((response) => response.ok ? response.json() : { enabled: false })
      .then((config: { enabled?: unknown }) => {
        if (active) setLocalDevelopmentAuth(config.enabled === true);
      })
      .catch(() => {
        if (active) setLocalDevelopmentAuth(false);
      });
  ...
  async function handleEmailLogin(event: FormEvent<HTMLFormElement>) {
    ...
    const callbackPath = safeInternalCallbackURL(
      searchParams.get("callbackURL"),
      window.location.origin,
    );
      ...
      <main className="relative z-10 grid min-h-screen place-items-center px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid w-full max-w-6xl overflow-hidden rounded-4xl border border-slate-200/80 bg-white/80 shadow-2xl shadow-slate-200/70 backdrop-blur-2xl dark:border-white/8 dark:bg-white/[0.035] dark:shadow-black/40 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <section className="p-8 sm:p-10 lg:p-12">
            ...
            {localDevelopmentAuth === null ? (
              <div
                aria-label={t("login.loading")}
                className="mt-8 flex min-h-[52px] w-full items-center justify-center"
              >
              ...
              <form className="mt-8 space-y-4" onSubmit={handleEmailLogin}>
                <div>
                  ...
                  <input
                    ...
                    autoComplete="email"
                    required
                    className="min-h-[48px] w-full rounded-xl border border-slate-200 bg-white px-4 text-sm text-slate-950 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/15 dark:border-white/10 dark:bg-white/6 dark:text-slate-100"
                  />
                </div>
                <div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex min-h-[52px] w-full items-center justify-center rounded-2xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-[#5e6ad2] dark:hover:bg-[#6b78df]"
                >
                  {loading ? (
