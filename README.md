...
Nexus includes an authenticated, human-in-the-loop AI operator with per-user OpenAI or Anthropic credentials, tenant-scoped CRM read tools, and explicit approval before application updates or remote MCP calls. Operator state and encrypted secrets require the Prisma/PostgreSQL deployment path; the optional Firestore adapter covers CRM application data only.

See the detailed [AI operator architecture](docs/architecture/ai-operator-console.md) and [threat model](docs/security/ai-operator-threat-model.md) for data flow, provider disclosure, persistence, and operational controls.
