      ...
      `}
		>
					...
					<div className="flex items-center gap-1">
						...
						{app.jobUrl && (
							<button
								...
								title={app.jobUrl}
								className="nexus-focus-ring shrink-0 rounded text-slate-400 transition hover:text-indigo-600 dark:hover:text-[#828fff]"
							>
								<svg
									className="w-3 h-3"
									fill="none"
			...
			<div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-400 dark:text-slate-500">
				...
				{app.appliedAt && (
					<span>{format(new Date(app.appliedAt), "dd.MM.yyyy")}</span>
				)}
				{followUpDate && (
					<span
			...
			{app.rating && (
				<div
					className="mt-2 text-yellow-400 text-xs tracking-tight"
					title={`${app.rating}/5`}
				>
					{"★".repeat(app.rating)}
					{"☆".repeat(5 - app.rating)}
...
export function KanbanView({ applications, onEdit }: KanbanViewProps) {
	...
	const [sortKey, setSortKey] = useState<KanbanSortKey>(() => {
		...
		return (
			(localStorage.getItem("kanban-sort") as KanbanSortKey) || "rating_desc"
		);
	});
						...
						<section key={status} className="space-y-2">
							...
							<div className="space-y-2">
								{grouped[status].map((app) => (
									<div
										key={app.id}
										className="overflow-hidden rounded-2xl bg-white/80 shadow-sm ring-1 ring-slate-200/80 dark:bg-white/[0.035] dark:ring-white/8"
									>
										<KanbanCard app={app} onEdit={onEdit} />
										<div className="flex items-center gap-2 border-t border-slate-100 p-2 dark:border-white/6">
											<button
												type="button"
												onClick={() => onEdit(app)}
											...
											<select
												...
												className="nexus-input nexus-target min-w-0 flex-1"
												aria-label={`${tk("change_status")}: ${app.company}`}
											>
