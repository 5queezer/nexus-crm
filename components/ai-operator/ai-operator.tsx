...
function ProposalCard({ proposal, busy, onApprove, onReject }: { proposal: ActionProposal; busy: boolean; onApprove: () => void; onReject: () => void }) {
  ...
  return <div id={`proposal-${proposal.id}`} className="ml-10 overflow-hidden rounded-2xl border border-indigo-200 bg-indigo-50/50 shadow-sm dark:border-indigo-500/20 dark:bg-indigo-500/[0.06]">
    ...
    <div className="space-y-2 p-4">
      ...
      {mcpPayload?.connectorName && <div className="text-xs"><span className="font-medium text-slate-500">{t("proposal_connector")}</span><span className="ml-3 font-semibold text-slate-800 dark:text-slate-200">{mcpPayload.connectorName}</span></div>}
      {mcpPayload?.connectorUrl && <div className="text-xs"><span className="font-medium text-slate-500">{t("proposal_destination")}</span><span className="ml-3 break-all font-mono text-slate-800 dark:text-slate-200">{mcpPayload.connectorUrl}</span></div>}
