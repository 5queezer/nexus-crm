...
describe("AiOperator", () => {
	...
	it("renders the exact sanitized MCP tool arguments before approval", async () => {
		...
		expect(
			await screen.findByText("I found one follow-up that needs attention."),
		).toBeTruthy();
	...
	it("refreshes expired proposal and thread state after an approval error", async () => {
		...
		vi.spyOn(globalThis, "fetch").mockImplementation((input, init) => {
			...
			if (url.endsWith("/api/agent/threads"))
				return json({ threads: [baseThread] });
			if (url.endsWith("/api/agent/threads/thread-1")) {
				threadRequests += 1;
				return json({
					thread:
						threadRequests > 1
							? { ...baseThread, title: "Expired review" }
							: baseThread,
				});
			}
			if (url.includes("/api/agent/proposals?threadId=")) {
			...
			if (
				url.endsWith("/api/agent/proposals/proposal-1/approve") &&
				init?.method === "POST"
			) {
		...
		vi.spyOn(globalThis, "fetch").mockImplementation((input, init) => {
			...
			if (url.includes("/api/agent/proposals?threadId=")) {
				...
				if (proposalRequests > 1)
					return json({ error: "Could not refresh approvals" }, 503);
				return json({
					proposals: [
						{
							id: "proposal-1",
	...
	it("labels connector settings, exposes expansion state, and refreshes tools after edits", async () => {
		...
		vi.spyOn(globalThis, "fetch").mockImplementation((input, init) => {
			...
			if (url.endsWith("/api/agent/credentials"))
				return json({ providers: [], credentials: [] });
			if (url.endsWith("/api/agent/threads")) return json({ threads: [] });
			if (url.endsWith("/api/agent/connectors") && !init?.method)
				return json({ connectors: [connector] });
			if (url.endsWith("/api/agent/connectors/connector-1/tools")) {
				toolRequests += 1;
				return json({ tools: [{ name: `search-v${toolRequests}` }] });
			}
			if (
				url.endsWith("/api/agent/connectors/connector-1") &&
				init?.method === "PUT"
			) {
		...
		expect(expander.getAttribute("aria-controls")).toBe(
			"connector-tools-connector-1",
		);
		...
		expander = await screen.findByRole("button", {
			name: "Hide tools for Updated tools",
		});
		await user.click(expander);
		await user.click(
			screen.getByRole("button", { name: "Show tools for Updated tools" }),
		);
	...
	it("shows starter actions when the authenticated user already configured a model", async () => {
		...
		expect(
			screen.getByRole("button", { name: /Summarize my pipeline/ }),
		).toBeTruthy();
		expect(screen.getByText("Private · your model key")).toBeTruthy();
	});

	it("selects the first saved non-OpenAI credential and keeps initial loading one-shot", async () => {
		...
		await user.click(
			screen.getAllByRole("button", { name: "Operator settings" }).at(-1)!,
		);
		await user.keyboard("{Escape}");
		expect(credentialGets).toBe(1);
	});

	it("resets a completed discovery catalog when the API key changes", async () => {
		vi.spyOn(globalThis, "fetch").mockImplementation((input) => {
			const url = String(input);
			if (url.endsWith("/api/agent/credentials"))
				return json({
			...
			if (url.endsWith("/api/agent/provider-models"))
				return json({
					models: [
						{ id: "key-a-model", label: "Key A model", description: "" },
					],
				});
			return json({ error: "not found" }, 404);
		});
		...
		await user.type(keyInput, "key-bbbb-1234");
		expect(screen.queryByRole("option", { name: "Key A model" })).toBeNull();
		expect(
			(
	...
	it("ignores stale discovery results after the API key changes and disables save while pending", async () => {
		...
		vi.spyOn(globalThis, "fetch").mockImplementation((input) => {
			...
			if (url.endsWith("/api/agent/credentials"))
				return json({
					providers: [
						{ id: "openai", label: "OpenAI", authMode: "api_key", models: [] },
					],
					credentials: [],
				});
			if (url.endsWith("/api/agent/threads")) return json({ threads: [] });
			if (url.endsWith("/api/agent/provider-models")) return discovery;
			return json({ error: "not found" }, 404);
		});
		const user = userEvent.setup();
		renderOperator();
		...
		await waitFor(() =>
			expect((modelInput as HTMLInputElement).value).toBe("manual-model"),
		);
		expect(screen.queryByRole("option", { name: "Key A model" })).toBeNull();
	});

	it("keeps a configured model missing from discovery and saves it without requiring the key again", async () => {
					...
					providers: [
						{
							...
							authMode: "api_key",
							models: [
								{ id: "new-model", label: "New model", description: "" },
							],
						},
					],
					credentials: [
		...
		expect(savedBody).toEqual({ provider: "openai", model: "retired-model" });
	});
});
