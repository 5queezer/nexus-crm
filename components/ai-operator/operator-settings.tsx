...
export function OperatorSettings({ providers, credentials, onCredentialsChange, onClose }: SettingsProps) {
  ...
  async function loadTools(connector: Connector) {
    ...
    setError("");
    try {
      const result = await apiJson<{ tools: McpTool[]; health: Pick<Connector, "lastCheckedAt" | "lastStatus" | "lastErrorCode"> }>(`/api/agent/connectors/${connector.id}/tools`);
      setTools((current) => ({ ...current, [connector.id]: result.tools }));
      setConnectors((current) => current.map((item) => item.id === connector.id ? { ...item, ...result.health } : item));
    } catch (reason) {
