            ...
            <div className="space-y-2">
              ...
              {connectors.map((connector) => (
                <div key={connector.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-white/8 dark:bg-white/[0.02]">
                  <div className="flex items-center gap-3 p-3"><span className={`h-2 w-2 rounded-full ${connector.enabled ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-700"}`} /><button className="min-w-0 flex-1 text-left" onClick={() => loadTools(connector)}><span className="block truncate text-sm font-medium text-slate-900 dark:text-white">{connector.name}</span><span className="block truncate text-[11px] text-slate-500">{connector.url}</span></button><button onClick={() => setConnectorForm({ id: connector.id, name: connector.name, url: connector.url, authorization: "", enabled: connector.enabled })} className="rounded-lg px-2 py-1 text-xs text-slate-500 hover:bg-slate-100 dark:hover:bg-white/5">{t("edit")}</button><button onClick={() => deleteConnector(connector.id)} disabled={busy === connector.id} className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10"><Trash2 className="h-3.5 w-3.5" /></button><button onClick={() => loadTools(connector)} className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5">{expandedConnector === connector.id ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</button></div>
