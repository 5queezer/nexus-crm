...
export type CredentialMetadata = Omit<CredentialRecord, "userId" | "encryptedApiKey">;
...
export const prismaCredentialRepository: CredentialRepository = {
  ...
  async remove(userId, provider) {
    const result = await prisma.llmCredential.deleteMany({ where: { userId, provider } });
...
export async function loadCredentialSecret(
  ...
  getProviderConfig(record.provider, record.defaultModel);
  return {
    provider: record.provider,
