...
class MemoryConnectorRepository implements ConnectorRepository {
	...
	async upsert(
		input: Omit<ConnectorRecord, "id" | "createdAt" | "updatedAt"> & {
			id?: string;
		},
	) {
...
describe("per-user MCP connectors", () => {
	...
	it("never carries authorization to a different connector origin", async () => {
		...
		expect(
			(await getConnectorSecret(repository, "user-a", saved.id))?.authorization,
		).toBeNull();
	});
	...
	it("records sanitized health without changing the connector configuration version", async () => {
		...
		const health = await recordConnectorHealth(
			repository,
			"user-a",
			saved.id,
			"failed",
	...
	it("scopes list, secret lookup, and delete to the authenticated user", async () => {
		...
		expect(
			await getConnectorSecret(repository, "user-a", saved.id),
		).toMatchObject({
			authorization: "Bearer user-a-token",
		});
	});
...
describe("reviewed MCP calls", () => {
	it("canonicalizes and validates reviewed arguments", () => {
		...
		const reordered = canonicalizeMcpCall(
			{ query: "platform", limit: 5 },
			schema,
		);
		...
		expect(() =>
			canonicalizeMcpCall(
				...
				{
					...schema,
					properties: { ...schema.properties, api_key: { type: "string" } },
				},
			),
		...
		const sensitiveValues = [
			`Bearer ${"credential".repeat(3)}`,
			"Basic dGVzdDp0ZXN0",
			`${"sk"}-${"a".repeat(32)}`,
			`${"ghp"}_${"b".repeat(36)}`,
			`${"eyJhbGciOiJIUzI1NiJ9"}.${"eyJzdWIiOiIxMjM0NTY3ODkwIn0"}.${"c".repeat(32)}`,
			`-----BEGIN PRIVATE KEY-----\n${"d".repeat(64)}\n-----END PRIVATE KEY-----`,
		];
		for (const sensitive of sensitiveValues) {
			expect(() => canonicalizeMcpCall({ query: sensitive }, schema)).toThrow(
				"MCP arguments contain a sensitive value",
			);
		}
	});
});
...
describe("pinned MCP transport", () => {
	it("uses the validated address dispatcher and rejects oversized bodies while streaming", async () => {
		const destination = await resolveMcpDestination(
...
describe("bounded MCP discovery", () => {
	...
	it("namespaces discovered tools and always closes the client", async () => {
		...
		expect(tools.map((tool) => tool.name)).toEqual([
			"research_tools__search",
			"research_tools__read",
		]);
		expect(close).toHaveBeenCalledOnce();
		expect(listTools).toHaveBeenCalledWith(
			expect.objectContaining({
				options: expect.objectContaining({ timeout: 5_000 }),
			}),
		);
	});
		...
		await expect(
			discoverMcpTools(
				...
				},
				{
					...
					connect: async () => ({
						listTools: async () => ({
							tools: Array.from({ length: 51 }, (_, i) => ({
								name: `tool-${i}`,
							})),
						}),
						close,
